...
export class ConversationContextManager implements IContextManager {
  ...
  async optimizeContext(
    ...
    const relevanceScore = this.calculateContextRelevance(
      turnCount,
      totalTokens,
    );
    ...
    if (shouldTruncate) {
      ...
      recommendedSummary = this.generateContextSummary(
        turnsToSummarize,
        turnCount,
      );
    }
  ...
  async truncateContext(
    ...
    const summaryCreated = this.generateContextSummary(
      turnsToRemove,
      turnsToRemove + 10,
    );
  ...
  async calculateRelevanceScore(_conversationId: string): Promise<number> {
    // This would typically analyze conversation content for relevance
    // For now, return a baseline relevance score
    return 0.8;
  }

  async summarizeContext(_conversationId: string): Promise<string> {
...
export class MockContextManager implements IContextManager {
  private readonly mockResults: Map<string, any> = new Map();
