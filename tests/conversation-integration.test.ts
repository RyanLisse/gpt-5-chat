...
describe('AGENT 4: Conversation Integration Tests - Multi-Turn Flows', () => {
  ...
  beforeEach(() => {
    ...
    conversationManager = new ConversationStateManager(persistenceProvider, contextManager);
  });
  ...
  describe('Complete Multi-Turn Conversation Flows', () => {
    it('should handle a complete 5-turn conversation with response_id chaining', async () => {
      ...
      const conversation = await conversationManager.createConversation('user_integration_1');
      expect(conversation.userId).toBe('user_integration_1');
      expect(conversation.contextMetadata?.turnCount).toBe(0);

      // Turn 1: Initial message
      const turn1Request = await conversationManager.continueConversation(null, 'Hello, I need help with OpenAI API integration.');
      ...
      const turn1Response: ConversationResponse = {
        ...
        content: 'I can help you with OpenAI API integration. What specific aspects do you need assistance with?',
      };
      await conversationManager.updateConversationWithResponse(conversation.conversationId, turn1Response);

      // Verify conversation state after turn 1
      let updatedState = await conversationManager.getConversationState(conversation.conversationId);
      ...
      const turn2Request = await conversationManager.continueConversation('resp_turn1_integration', 'I want to implement stateful conversations using response IDs.');
      ...
      const turn2Response: ConversationResponse = {
        ...
        content: 'Great! Stateful conversations with response IDs allow you to maintain context efficiently. You can use the `store: true` parameter.',
      };
      await conversationManager.updateConversationWithResponse(conversation.conversationId, turn2Response);

      // Turn 3-5: Continue the conversation
      const turns = [
        { input: 'How do I handle response_id references?', responseId: 'resp_turn3_integration', content: 'You reference the previous response ID in the `previousResponseId` field of your request.' },
        { input: 'What about context optimization?', responseId: 'resp_turn4_integration', content: 'Context optimization helps manage token limits by truncating or summarizing older conversation turns.' },
        { input: 'Perfect, thank you!', responseId: 'resp_turn5_integration', content: 'You\'re welcome! Feel free to ask if you need more help with implementation.' }
      ];

      let lastResponseId = 'resp_turn2_integration';
      for (const turn of turns) {
        const request = await conversationManager.continueConversation(lastResponseId, turn.input);
        ...
        await conversationManager.updateConversationWithResponse(conversation.conversationId, response);
    ...
    it('should preserve conversation state across manager instances (persistence test)', async () => {
      ...
      const manager1 = new ConversationStateManager(persistenceProvider, contextManager);
      const conversation = await manager1.createConversation('user_persistence_test');
      
      const turn1Response: ConversationResponse = {
        id: 'resp_persistence_test',
        content: 'Initial response for persistence test',
      };
      await manager1.updateConversationWithResponse(conversation.conversationId, turn1Response);

      // Simulate application restart with new manager instance
      const manager2 = new ConversationStateManager(persistenceProvider, contextManager);
      
      // Retrieve conversation state with new manager
      const recoveredState = await manager2.getConversationState(conversation.conversationId);
      ...
      const continuedRequest = await manager2.continueConversation('resp_persistence_test', 'Continuing after restart');
    ...
    it('should handle context optimization during long conversations', async () => {
      const conversation = await conversationManager.createConversation('user_context_test');
      
      // Setup context manager to trigger optimization
      contextManager.setMockResult('optimizeContext', conversation.conversationId, {
        shouldTruncate: true,
        relevanceScore: 0.85,
        recommendedSummary: 'User discussed OpenAI API implementation details and best practices.',
        tokensToRemove: 2000,
      });
      
      // Simulate long conversation requiring optimization
      await conversationManager.optimizeConversationContext(conversation.conversationId, {
      ...
      const optimizedState = await conversationManager.getConversationState(conversation.conversationId);
    ...
    it('should maintain conversation metadata consistency across operations', async () => {
      const conversation = await conversationManager.createConversation('user_metadata_test');
      
      // Track metadata evolution through multiple turns
      const responses: ConversationResponse[] = [
        { id: 'resp_meta_1', content: 'First response with some content to track tokens' },
        { id: 'resp_meta_2', content: 'Second response adding more context and token count' },
        { id: 'resp_meta_3', content: 'Third response for comprehensive metadata testing' },
      ];

      for (let i = 0; i < responses.length; i++) {
        await conversationManager.updateConversationWithResponse(conversation.conversationId, responses[i]);
        
        const currentState = await conversationManager.getConversationState(conversation.conversationId);
        expect(currentState?.contextMetadata?.turnCount).toBe(i + 1);
        expect(currentState?.previousResponseId).toBe(responses[i].id);
        expect(currentState?.contextMetadata?.totalTokens).toBeGreaterThan(i * 50); // Growing token count
        ...
        expect(new Date(currentState?.updatedAt!).getTime()).toBeGreaterThanOrEqual(
    ...
    it('should handle concurrent conversation updates safely', async () => {
      const conversation = await conversationManager.createConversation('user_concurrent_test');
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should handle context manager errors gracefully', async () => {
      ...
      const brokenContextManager = {
        optimizeContext: vi.fn().mockRejectedValue(new Error('Context optimization failed')),
        truncateContext: vi.fn(),
        calculateRelevanceScore: vi.fn(),
        summarizeContext: vi.fn(),
      };

      const managerWithBrokenContext = new ConversationStateManager(persistenceProvider, brokenContextManager);
      const conversation = await managerWithBrokenContext.createConversation('user_error_test');

      await expect(
        managerWithBrokenContext.optimizeConversationContext(conversation.conversationId, {
          turnCount: 10,
          totalTokens: 5000,
          maxTokens: 3000,
        })
    ...
    it('should require context manager for context optimization', async () => {
      const managerWithoutContext = new ConversationStateManager(persistenceProvider);
      const conversation = await managerWithoutContext.createConversation('user_no_context');

      await expect(
        managerWithoutContext.optimizeConversationContext(conversation.conversationId, {
          turnCount: 10,
          totalTokens: 5000,
          maxTokens: 3000,
        })
      ).rejects.toThrow('ContextManager not provided - cannot optimize context');
    });
  });
});